...
'''
使用说明：
如果你选择手动配redis_url，请配置REDIS_URL，如果选择配到环境变量，请把REDIS_URL留空
单例模式，请不要自行实例化类生产对象
...
例子：
proxy_pool.get()
push_blacklist和change_ip目前为同一个接口，请在新项目中使用push_blacklist，主要用于拉黑ip，把ip加入黑名单，拉黑后
会自动通知主服务器，加快ip更换速度，并且再调用get函数的时候不会再出现拉黑的ip
例子：
proxy_pool.push_blacklist('http://121.226.59.116:10000')
'''
REDIS_URL = 'redis://10.0.54.54:6378'
setting_timing = 60*60*24
...
class ProxyPool(object):
    ...
    redis_url = REDIS_URL or os.environ.get('REDIS_URL', None)
    ...
    def __init__(self):
        ...
        self.logger.info(''.center(50, "*"))
        assert self.redis_url is not None, 'redis_url is None'
    ...
    def get(self, _times=0):
        ...
        if self.__pool:
            ...
            if ip_port['last_time'] >= int(now) and ip_port['ip'] not in self.__blacklist:
                tmp_ip = "{0}:{1}".format(ip_port['ip'], ip_port['port'])
                return tmp_ip
            else:
                self.logger.debug('get(): ip in blacklist, or living is None, again'.format(ip_port))
                # self.__get_all_ip()
